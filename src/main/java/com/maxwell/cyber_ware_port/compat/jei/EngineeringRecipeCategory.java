...
public class EngineeringRecipeCategory implements IRecipeCategory<EngineeringRecipe> {
    ...
    public @NotNull Component getTitle() {
        return Component.translatable("gui.cyberware_unofficial.deconstruct");
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, EngineeringRecipe recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.CATALYST, 15, 53)
                ...
                .addTooltipCallback((view, tooltip) ->
                        tooltip.add(Component.translatable("gui.cyberware_unofficial.need_paper").withStyle(ChatFormatting.GRAY))
                );
        int outputX = 71;
        int outputY = 17;
        for (int i = 0; i < Math.min(recipe.getOutputs().size(), 6); i++) {
            EngineeringRecipe.OutputEntry entry = recipe.getOutputs().get(i);
        ...
        if (bpChance > 0 && inputIng.getItems().length > 0) {
            ...
            if (!blueprint.isEmpty()) {
                builder.addSlot(RecipeIngredientRole.OUTPUT, 115, 53)
                        ...
                        .addTooltipCallback((view, tooltip) ->
                                tooltip.add(Component.translatable("gui.cyberware_unofficial.blueprint_chance", String.format("%.0f", bpChance * 100)).withStyle(ChatFormatting.BLUE))
                        );
            }
        }
    }
