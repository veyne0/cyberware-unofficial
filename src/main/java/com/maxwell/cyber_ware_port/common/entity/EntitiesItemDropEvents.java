...
public class EntitiesItemDropEvents {
    ...
    public static void onLivingDrops(LivingDropsEvent event) {
        ...
        // 可拆解生物:击杀按概率随机掉落生物部位(概率与赛博义体掉落一致)
        if (com.maxwell.cyber_ware_port.common.util.MobSurfaceParts.canDropParts(entity.getType())) {
            dropRandomMobParts(event, entity);
            return;
        }
        ...
        int looting = getLootingLevel(attacker);
    ...
    /**
     * 可拆解生物击杀掉落:基础概率 10%+抢夺 5%(武士刀 50%+抢夺 10%),每次随机掉一个部位。
     * 例外:史莱姆仅最小体型 100% 掉落;凋零必定随机掉头/躯干之一,两个都掉的概率 2.5%。
     */
    private static void dropRandomMobParts(LivingDropsEvent event, LivingEntity entity) {
        RandomSource random = entity.getRandom();
        // 史莱姆:只有分裂到底的最小体型(1)掉落部位,且 100% 掉率;大史莱姆不掉
        // 注意:MagmaCube 继承自 Slime,需要排除,否则最小岩浆怪也会掉史莱姆部位
        if (entity instanceof net.minecraft.world.entity.monster.Slime slime
            ...
            if (slime.getSize() == 1) {
                addMobPartDrop(event, entity, random);
            }
            return;
        }
        // 凋零:必定掉一个随机部位,2.5% 概率两个都掉(不受抢夺/武士刀影响)
        ...
        int looting = getLootingLevel(attacker);
        float baseChance = com.maxwell.cyber_ware_port.common.util.MobSurfaceParts.dropChance(entity.getType());
        if (random.nextFloat() < baseChance + (looting * 0.05f)) {
            addMobPartDrop(event, entity, random);
        }
        if (attacker != null && attacker.getMainHandItem().is(ModItems.KATANA.get())) {
            if (random.nextFloat() < 0.50f + (looting * 0.10f)) {
                addMobPartDrop(event, entity, random);
            }
        }
    }

    private static void addMobPartDrop(LivingDropsEvent event, LivingEntity entity, RandomSource random) {
        // 充能苦力怕掉落闪电苦力怕部位(贴图/技能变体,同一部位表)
        ResourceLocation override = entity instanceof net.minecraft.world.entity.monster.Creeper c && c.isPowered()
                ? new ResourceLocation("minecraft", "lightning_creeper") : null;
        ItemStack part = com.maxwell.cyber_ware_port.common.util.MobSurfaceParts.randomDrop(entity.getType(), random, override);
        if (part.isEmpty()) return;
        event.getDrops().add(new ItemEntity(entity.level(), entity.getX(), entity.getY(), entity.getZ(), part));
    }

    private static int getLootingLevel(LivingEntity attacker) {
        return (attacker != null)
                ? EnchantmentHelper.getEnchantmentLevel(Enchantments.MOB_LOOTING, attacker) : 0;
    }
